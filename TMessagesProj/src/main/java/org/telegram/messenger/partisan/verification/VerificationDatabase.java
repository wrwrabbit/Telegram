...
public class VerificationDatabase {
    ...
    public void createDatabase(int tryCount, boolean fromBackup) {
        ...
        try {
            ...
            if (createTable) {
                ...
                database.executeFast("CREATE TABLE verified(storage_id INTEGER, chat_id INTEGER, username TEXT, type INTEGER, PRIMARY KEY(storage_id, chat_id) FOREIGN KEY(storage_id) REFERENCES verified_storage(storage_id) ON DELETE CASCADE);").stepThis().dispose();
    ...
    private void fill_database() throws Exception {
        ...
        try {
            ...
            cursor = database.queryFinalized("SELECT last_insert_rowid()");
            int storage_id;
            if (cursor.next()) {
    ...
    public List<Storage> getStorages() {
        ...
        try {
            ...
            cursor = database.queryFinalized("SELECT storage_id, storage_name, chat_username, chat_id, last_check_time, last_post_id FROM verified_storage");
            while (cursor.next()) {
                Storage storage = new Storage();
                storage.chatId = cursor.intValue(0);
                storage.storageName = cursor.stringValue(1);
                storage.chatUsername = cursor.stringValue(2);
                storage.chatId = cursor.longValue(3);
                storage.lastCheckTime = cursor.longValue(4);
                storage.lastPostId = cursor.intValue(5);
    ...
    public int addStorage(String name, String username, int chat_id) {
        ensureDatabaseCreated();
        if (database == null) {
            return -1;
        }
        ...
        try {
            ...
            statement.bindInteger(3, chat_id);
            statement.bindInteger(4, 0);
            statement.bindInteger(5, 0);
            statement.step();
            statement.dispose();
    ...
    public void putChats(int storageId, List<ChatInfo> chats) {
        postRunnable(() -> {
            try {
                ensureDatabaseCreated();
                database.beginTransaction();
                SQLitePreparedStatement statement = database.executeFast("REPLACE INTO verified(storage_id,chat_id,username,type) VALUES (?, ?, ?, ?)");

                for (ChatInfo chat : chats) {
                    statement.requery();
                    statement.bindInteger(1, storageId);
                    statement.bindLong(2, chat.chatId);
                    statement.bindString(3, chat.username);
                    statement.bindInteger(4, chat.type);
                    statement.step();
                }
    ...
    public void deleteChats(int storageId, List<Long> chatIds) {
        postRunnable(() -> {
            try {
                ensureDatabaseCreated();
                database.beginTransaction();
                SQLitePreparedStatement statement = database.executeFast("DELETE FROM verified WHERE storage_id = " + storageId + " AND chat_id = ?");

                for (Long chatId : chatIds) {
                    statement.requery();
                    statement.bindLong(1, chatId);
                    statement.step();
                }
            } catch (Throwable ignore) {
            } finally {
